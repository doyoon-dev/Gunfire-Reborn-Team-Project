...
public class MonsterMelee : Monster
{
    ...
    protected override void OnEnable()
    {
        base.OnEnable();
        m_isDead = false;
        m_traceDist = 300f;//추적사정거리_근거리
        m_battleDist = 10f;//전투거리_근거리
        m_attackDist = 6f;//공격사정거리_근거리
        m_arrangeDist = 0f;//정비거리_근거리는0
        m_maxShotDelay = 3f;//공격딜레이_근거리
        m_status = new Status(1000, 25, 500, 0, 0);
        
        m_playerMask = LayerMask.GetMask("Player");
        m_curState = eCurrentState.eIdle;

        m_monSpeed = 20f; //이동속도_근거리(유니티 기본설정은 3.5)
        m_isBoss = false;   //보스인지 아닌지
        m_hasSetDestination = false;
        m_isMeleeMonster = true;
        m_monsterName = new string("창 병");
    ...
    void MeleeAttack()
    {        
        Collider[] targets = Physics.OverlapSphere(this.transform.position, m_attackDist + 2f, m_playerMask);
